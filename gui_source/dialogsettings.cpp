    ...
    ui(new Ui::DialogSettings)
{
    ...
    foreach (const QSerialPortInfo &serialPortInfo, QSerialPortInfo::availablePorts())
        ...
        portDescription = serialPortInfo.description();
            if (portDescription.contains("Enhanced", Qt::CaseInsensitive) || portDescription.contains("User", Qt::CaseInsensitive))
            {
                ui->comboBox_UserUART->addItem(serialPortInfo.portName());
            }

            if (portDescription.contains("Standard", Qt::CaseInsensitive) || portDescription.contains("Data", Qt::CaseInsensitive))
            {
