...
//    for(int i=0;i<HEART_RATE_EST_MEDIAN_FLT_SIZE;++i){
//        heartRateBuffer.insert(i,65);
//    }
    // Displays TI Logo
    myfile.open("C:/Users/ISIFAOUI/Desktop/68xx_vital_signs/Dataset/"+std::to_string(timeOfDay)+".csv");//zidtha
...
void MainWindow::processData()
{
    ...
    // Kalman Filter
    static float Pk = 1;
    static float xk = 0, xb = 17;   // State Variable
...
//    timer.start();
    ...
    //int rrTemp = rrcount % NUM_PTS_RRSIGNAL_PLOT;
    if (dataStartIndex == -1)
    {
...
if (APPLY_KALMAN_FILTER)
{
   ...
   CM_combined = heartRate_2hz_CM + 10*heartRate_xCorr_CM+CM2;//+cm4hz
  // CM_combined/=2;
...
qDebug()<<heartRateEstDisplay;
   ...
   //qDebug()<<outSumEnergyHeartWfm<<';'<<heartRate_Out<<"*"<<BreathingRate_FFT<<";"<<KF_Gain<<";"<<heartRate_4Hz_CM<<";"<<diff;//heartRate_4Hz_CM<<";"<<heartRate_Out;


   //if(abs(BreathingRate_FFT-KF_Gain)>0.05)// || heartRate_4Hz_CM!=heartRate_Out)
       //qDebug()<<";"<<-1;
       //exit(-1);
 ...
 {
     if(heartRate_4Hz_CM > 0){
         for(int i=0;i<NUM_PTS_RRSIGNAL_PLOT-1;++i){
             jjsignalBuffer[i]=jjsignalBuffer[i+1];
         }
        // xRRsignal[rrTemp]=rrTemp;
         jjsignalBuffer[NUM_PTS_RRSIGNAL_PLOT-1] = heartRate_4Hz_CM*50;
     }
...
//   ui->plot_RangeProfile->yAxis->rescale();
   ...
   if( heartRateEstDisplay!=outSumEnergyHeartWfm ) myfile<<';'<<-1;
   QString myString_HeartRate;
   ui->lcdNumber_HeartRate->setDigitCount(3);
   myString_HeartRate.sprintf("%1.0f",heartRateEstDisplay);//heartRate_Out);
